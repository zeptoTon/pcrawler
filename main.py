...
class Crawler:
  ...
  def _content_router(self, document, l_property):
    ...
    if l_property["type"] == "text":
      self.result[l_property["name"]] = self._extract_text_node(document, l_property["css_path"], l_property["format"], l_property["multiple"])
    elif l_property["type"] == "sizeof":
      self.result[l_property["name"]] = self._extract_size_node(document)
    elif l_property["type"] == "link":
      self._extract_link_node(document, l_property["css_path"], l_property["nested_properties"])
  ...
  def start(self):
    ...
    content = self.opener.open(url).read()
    items = BeautifulSoup(content, "html.parser").select(self.config["items_css_path"])
    results = []
    for item in items:
      self.result = {}
